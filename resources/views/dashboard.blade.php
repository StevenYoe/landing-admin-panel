...
@section('content')
<!-- First Cards Row (Show only for non-HR divisions) -->
@if($hasContentAccess || (!$hasHRAccess && !$isSuperAdmin))
...
<!-- Vacancy Cards Row (Show only for HR and superadmin) -->
@if($hasHRAccess)
    @if(!$hasContentAccess)
    <!-- HR-only Dashboard (for HR division) -->
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
    @else
        <!-- Additional HR cards for superadmin -->
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4 mt-6">
    @endif
...
<div class="grid grid-cols-1 gap-6 mt-6 lg:grid-cols-2">
    <!-- Content Tables (Show only for non-HR divisions) -->
    @if($hasContentAccess || (!$hasHRAccess && !$isSuperAdmin))
    ...
    <!-- HR Tables (Show only for HR and superadmin) -->
    @if($hasHRAccess)
